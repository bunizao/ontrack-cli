...
async function update(testCase: GoldenCase, result: CliExecution): Promise<void> {
  ...
  if (metadata.kind === "python_oracle" && testCase.oracleOutput !== undefined) {
    throw new Error(`${testCase.name} has Python command output and cannot be regenerated by TypeScript`);
  }
  await Promise.all([
...
export async function test_command_goldens_match_or_regenerate_mechanically(): Promise<void> {
  ...
  assert.equal(cases.length, 19, "the compatibility corpus should retain its 19-case matrix");
...
export async function test_every_parser_command_has_a_golden_case(): Promise<void> {
  ...
  const parserCommands = [...new Set([...help.stdout.matchAll(/^  (auth (?:check|login)|[a-z][a-z-]*)(?:\s+<[^>]+>)?\s+/gmu)]
    .map((match) => match[1]!)
    .filter((command) => command !== "auth login" && command !== "resources" && command !== "task"))]
    .sort();
  ...
  assert.match(help.stdout, /^  task sheet\s+/mu, "task sheet downloads are covered by application and subprocess tests");
  assert.match(help.stdout, /^  task resources\s+/mu, "task resource downloads are covered by application and subprocess tests");
  assert.match(help.stdout, /^  chats send\s+/mu, "chat sending is covered by confirmation and subprocess tests");
